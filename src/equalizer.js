...
const Equalizer = (function () {
    let config = {
        ...
        GraphicEQRawFrequences: new Array(
            Math.ceil(Math.log(20000 / 20) / Math.log(1.0072))
        )
            .fill(null)
    ...
    let strip = function (filters) {
        ...
        return filters.map((f) => ({
            ...
            gain: Math.min(
                Math.max(Math.floor(f.gain * 10) / 10, minGain),
                maxGain
            ),
        }));
    };
    ...
    let optimize = function (fr, frTarget, filters, iteration, dir) {
        ...
        if (!dir) {
            ...
            for (let i = 0; i < filters.length - 1; ) {
                ...
                let f2 = filters[i + 1];
                if (
                    Math.abs(f1.freq - f2.freq) <= freq_unit(f1.freq) &&
